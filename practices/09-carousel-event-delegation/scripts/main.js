{
  ...
  // ✅ 인디케이터 클릭 이벤트 위임 방식으로 수정
  indicatorWrapper.addEventListener('click', (event) => {
    const clickedIndicator = event.target.closest('.carousel__indicator')
    if (!clickedIndicator || !indicatorWrapper.contains(clickedIndicator)) return

    const selectedIndex = Array.from(indicators).indexOf(clickedIndicator)
    ...
    selectedIndicator.classList.remove(SELECTED_CLASSNAME)
    clickedIndicator.classList.add(SELECTED_CLASSNAME)
